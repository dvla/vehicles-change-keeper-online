@tag
  Feature: Business keeper page filed validation
    ...
    Scenario: - Business name - invalid characters
      ...
      When  the user clicks on the no email radio button
      When  the user select the submit control
      Then  the user will receive an error message "Must be between 2 and 30 characters, contain at least one alpha character and only contain valid characters (a-z, A-Z, 0-9, &, -,(), "",/, ‘ and , or .). The following characters cannot be used at the start of business name (&,-,(), ", /, ‘ and , or .)"
