...
Feature:
  ...
  Scenario: Date of sale - Confirm new keeper with a date of sale before the previous keeper end date
    ...
    Then the user will remain on the Date of Sale page and a warning will be displayed "The date you’ve entered is before the current keeper acquired the vehicle, which is shown on the V5C registration certificate (logbook). Please enter a correct date that’s later than the one shown on the logbook and select ‘Next’."
