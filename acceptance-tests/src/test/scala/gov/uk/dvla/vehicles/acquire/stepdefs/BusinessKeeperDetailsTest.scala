...
final class BusinessKeeperDetailsTest(webBrowserDriver: WebBrowserDriver) extends ScalaDsl with EN with WebBrowserDSL with Matchers {
  ...
  def there_will_be_a_data_entry_control_for_fleet_number_using_the_format_NNNNNN_or_NNNNN(): Unit = {
    ...
    //BusinessKeeperDetailsPage.fleetNumberField.value=  shouldEqual(true)
  ...
  /*@Given("^the user has entered an email address$")
  def the_user_has_entered_an_email_address(){

  }

  @When("^the email entered does not have a valid email pattern \"(.*?)\"$")
    def the_email_entered_does_not_have_a_valid_email_pattern(a:String)  {
  ...
  def an_error_message_is_displays(postcoderErrMsg:String)  {
